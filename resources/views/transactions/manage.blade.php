            ...
            <div class="col-sm-12">
                {!! Form::open(['url' => route('transactions.store', ['patient' => request()->input('patient')]), 'method' => 'POST', 'id' => 'trans']) !!}
                    <table class="table table-bordered" style="margin-top:80px;">
                        ...
                        <tbody>
                            <tr><td colspan="9">{{ $doctor }}</td></tr>
                            <tr>
                                <td colspan="2"></td>
                                <td>Brand</td><td>Generic Name</td><td>Frequency</td><td>Dosage</td><td>Duration</td><td>Quantity</td><td style="width:10%"></td>
                            </tr>
                            @foreach($presc AS $p)
                                <tr>
                                    <td colspan="2"></td>
                                    ...
                                    <td>
                                        {{ Form::hidden("prescription[{$p->id}][id]", $p->id) }}
                                        {{ Form::bsText("prescription[{$p->id}][quantity]") }}
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
